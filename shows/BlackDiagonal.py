...
class BlackDiagonals(object):
    def __init__(self, trimodel):
        self.name = "Black Diagonals"
        self.tri = trimodel
        self.time = 0
        self.speed = 0.1
        self.width = 12
    ...
    def next_frame(self):
        for i in range(12, 0, -1):  # total number of triangles
            for corner in all_left_corners():
                self.tri.set_cells(tri_shape(corner, i), (0, 0, 0))
    ...
    def draw_background(self):
        for i in range(10, 0, -2):  # total number of triangles
            for corner in all_left_corners():
                self.draw_corner(corner, i)

    def draw_corner(self, corner, i):
        grey = 255 * self.get_att(self.width, i, self.time)
        self.tri.set_cells(tri_shape(corner, i), (grey, grey, grey))



class BlackDiagonalsRed(BlackDiagonals):
    def draw_corner(self, corner, i):
        grey = 255 * self.get_att(self.width, i, self.time)
        self.tri.set_cells(tri_shape(corner, i), (grey, 0, 0))

class BlackDiagonalsBlue(BlackDiagonals):
    def draw_corner(self, corner, i):
        grey = 255 * self.get_att(self.width, i, self.time)
        self.tri.set_cells(tri_shape(corner, i), (0, 0, grey))
